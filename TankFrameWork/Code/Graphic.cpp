...
bool Graphic::InitD3D()
{
	...
	//CreateDevice(Thiết bị vẽ mặc định, Hỗ trợ phần cứng, Cửa sổ vẽ, (Không rõ), Thông số BlackBuffer, Con trỏ nhận dữ liệu sai khi tạo Device);
...
{
	//CenterMatrix là vị trí chuyển đổi theo tâm
	D3DXVECTOR2 CenterMatrix = position;

	//Đổi Góc sang rad
	float rad = angle * (3.14159265358979323846 / 180);

	//Lấy Matrix ban đầu
	D3DXMATRIX oldMatrix;
	this->D3Dsprite->GetTransform(&oldMatrix);

	//Tạo matrix trống
	D3DXMATRIX matrix; //Ma trận biến đổi 
	//Thêm thông số cho Matrix
	D3DXMatrixTransformation2D(
		&matrix, //Matrix trả về
		&CenterMatrix,	//Vị trí scale (Mặc định bằng vị trí Object)
		0,		//Góc scale
		&scale, //Vector scale
		&CenterMatrix, //Vị trí góc quay (Mặc định bằng vị trí Object)
		rad,		//Góc quay theo rad
		&transform);//vector tịnh tiến (dịch chuyển 1 đoạn từ vị trí ban đầu theo tích vô hướng 1 khoảng "transform")
	//Set lại matrix vào Sprite sau khi chuyển
	this->D3Dsprite->SetTransform(&matrix);

	//Vẽ sau khi chuyển matrix
	this->D3Dsprite->Draw(texture, &source, &D3DXVECTOR3(center.x, center.y, 0), &D3DXVECTOR3(position.x, position.y, 0), color);

	//Thiết lập lại Ma trận cũ để không ảnh hưởng tới tính toán khác
